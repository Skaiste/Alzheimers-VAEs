...
class ResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = nn.GroupNorm(num_groups=norm_num_groups, channels=in_channels, eps=norm_eps, affine=True)
        ...
        self.norm2 = nn.GroupNorm(num_groups=norm_num_groups, channels=out_channels, eps=norm_eps, affine=True)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.scale = 1 / math.sqrt(channels / self.num_heads)

        self.norm = nn.GroupNorm(num_groups=norm_num_groups, channels=channels, eps=norm_eps, affine=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.spatial_dims == 1:
            x = x.view(batch, channel, length).transpose(1, 2)
        if self.spatial_dims == 2:
        ...
        if self.spatial_dims == 1:
            x = x.transpose(-1, -2).reshape(batch, channel, length)
        if self.spatial_dims == 2:
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        preserve_spatial_dims: bool = False,
        ...
        self.preserve_spatial_dims = preserve_spatial_dims
        ...
        for i in range(len(channels)):
            ...
            if not is_final_block and not self.preserve_spatial_dims:
        ...
        blocks.append(
            nn.GroupNorm(num_groups=norm_num_groups, channels=channels[-1], eps=norm_eps, affine=True)
        )
        blocks.append(
            Convolution(
    ...
    def __init__(
        ...
        if with_nonlocal_attn is True:
            ...
            blocks.append(
                AttentionBlock(
                    spatial_dims=spatial_dims,
                    channels=reversed_block_out_channels[0],
                    norm_num_groups=norm_num_groups,
                    norm_eps=norm_eps,
                    use_flash_attention=use_flash_attention,
                )
        ...
        blocks.append(nn.GroupNorm(num_groups=norm_num_groups, channels=block_in_ch, eps=norm_eps, affine=True))
        blocks.append(
            Convolution(
                spatial_dims=spatial_dims,
                in_channels=block_in_ch,
...
class AutoencoderKLv2(nn.Module):
    ...
    Args:
        ...
        norm_num_groups: number of groups for the GroupNorm layers, channels must be divisible by this number.
        norm_eps: epsilon for the normalization.
        preserve_spatial_dims: if True, skip all down/upsampling operations and keep latent spatial sizes equal
            to input spatial sizes.
    ...
    def __init__(
        ...
        self.encoder = Encoder(
            ...
            preserve_spatial_dims=preserve_spatial_dims,
